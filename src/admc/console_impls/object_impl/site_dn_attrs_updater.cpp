...
void SiteDnAttrsUpdater::update_dns_records(AdInterface &ad, const QString &new_site_dn) {
    ...
    for (auto dns_zone_dn : {domain_zone_dn, forest_zone_dn}) {
        const QString filter1 = filter_CONDITION(Condition_Contains, ATTRIBUTE_DNS_RECORD_DC, site_name);
        const QString filter2 = filter_CONDITION(Condition_Contains, ATTRIBUTE_DNS_RECORD_DC, "_sites");
        const QString filter3 = filter_CONDITION(Condition_Equals, ATTRIBUTE_OBJECT_CLASS, CLASS_DNS_NODE);
        const QString res_filter = filter_AND({filter1, filter2, filter3});
