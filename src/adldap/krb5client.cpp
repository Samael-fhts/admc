...
void Krb5Client::get_tgt_with_keytab() {
    QString error;
    krb5_creds new_creds;

    krb5_error_code res = krb5_get_init_creds_keytab(context, &new_creds, principal, keytab, 0, NULL, NULL);
    if (res) {
        error = QCoreApplication::translate("Krb5Client", "Failed to get credentials with keytab");
        throw std::runtime_error(error.toUtf8().data());
    }
...
bool Krb5Client::keytab_is_available() const {
    if (keytab == NULL) {
        return false;
    }
