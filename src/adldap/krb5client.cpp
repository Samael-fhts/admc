...
Krb5Client::Krb5Client() : context(NULL), ccache(NULL), principal(NULL) {
    ...
    result = krb5_unparse_name(context, principal, &principal_str);
    if (result) {
        state = Krb5TgtState_Invalid;
...
void Krb5Client::authenticate(const QString &principal, const QString &password) {
    bool same_principal = principal == tgt_contents.principal;
...
void Krb5Client::refresh_tgt() {
    const QString error = QCoreApplication::translate("Krb5Client", "Failed to get valid TGT");
}
