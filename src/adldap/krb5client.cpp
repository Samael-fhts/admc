...
Krb5Client::Krb5Client() : context(NULL) {
...
Krb5Client *Krb5Client::instance() {

}

void Krb5Client::authenticate(const QString &principal, const QString &password) {
    const bool same_principal = principal == current_principal();

    switch (state) {
    case Krb5TgtState_Active:
    case Krb5TgtState_Expired:
        same_principal ? refresh_tgt(principal) : kinit(principal, password);
        break;
...
void Krb5Client::refresh_tgt(const QString &principal) {
    ...
    krb5_error_code res = krb5_get_renewed_creds(context, &new_creds, principal, ccache, NULL);
    if (res) {
    ...
    krb5_creds *creds_ptr = &creds;
    res = krb5_copy_creds(context, &new_creds, &creds_ptr);
    if (res) {
        krb5_free_cred_contents(context, &new_creds);
        throw std::runtime_error(error.toUtf8().data());
    }
...
void Krb5Client::load_default_cache() {
    krb5_error_code result;
...
void Krb5Client::load_custom_caches() {
